...
La composition des communautés de Radiolaires semble varier en fonction du mois de l'année.  
<ul>
  <li>Les Acanthaires sont présents tout au long de l'année mais sont particulièrement abondants (relativement aux autres ordres) entre avril et novembre. on voit un premier pic au milieu du graphe et un autre qui commence à droite.</li>
  <li>Les Nassellaires ne semblent pas dépendre de la saison, ils sont faiblement présents tout au long de l'année.</li>
  <li>Les groupes environnementaux sont surtout abondants en hiver entre novembre et mars-avril.</li>
  <li>Les Spumellaires sont présents pendant toute l'année. Leur abondance varie fortement entre les mois.</li>
</ul>
...
include_graphics("Images/abundance_plots.png")
```

On voit ici plus précisément les variations temporelles de chaque ordre.  
Certains comme Acantharea_4, RAD-B_X et sans doute Acantharea_E (pas assez de données pour être sûr) montrent une forte saisonnalité, leurs pics d'abondance se font toujours aux même mois.  
D'autres comme Acantharea_B et Radiolaria_XX présentent des pics d'abondance uniques qui ne se répètent pas entre les années.  
Pour les ordres Acantharea_B, Acantharea_D, Acantharea_F, RAD-A_X et RAD-B_X, un pic d'abondance est observé pour les derniers mois (avril-mai).
...
```{r, echo=FALSE, fig.retina=4}
include_graphics("Images/PC_plots.png")
```

Les variables temp, PO4, TIN, solar_radiation, wind_speed, PAR et UV montrent une saisonnalité marquée.  
Les variables salinity, chl, phaeo, silica, TIN, SI_TIN, SI_P et TIN_P montrent toutes un pic particulièrement élevé dans les derniers mois (avril-mai) ce qui correspond aux pics d'abondance vus avant chez Acantharea_B, Acantharea_D, Acantharea_F, RAD-A_X et RAD-B_X.
...
L'ACP ne représente que 50 % de la variablilité des données, ce qui est assez peu.  
